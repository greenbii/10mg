...
export class AdminWalletResolver implements Resolve<any> {
    ...
    async getData() {
        try {
            ...
            const resp = await this.appService.initiateHttpRequest('get', '/admin/supplier-balances', null, uToken).toPromise();
            if(resp) {
                if(resp.status === true) {
