...
export class DashboardSummaryResolver implements Resolve<any> {
    ...
    async getData() {
        try {
            ...
            const resp = await this.appService.initiateHttpRequest('get', '/admin/dashboard-summary', null, uToken).toPromise();
            if(resp) {
                if(resp.status === true) {
