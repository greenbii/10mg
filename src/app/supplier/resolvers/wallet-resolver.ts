...
export class SupplierWalletResolver implements Resolve<any> {
    ...
    async getData() {
        try {
            ...
            const resp = await this.appService.initiateHttpRequest('get', '/supplier/wallet', null, uToken).toPromise();
            if(resp) {
                if(resp.status === true) {
